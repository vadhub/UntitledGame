...
public class Tower extends GameObject {
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        // Устанавливаем новые координаты для отрисовки иконки
        this.x = x;
        this.y = y;

        // Отрисовываем уменьшенную версию башни
        Graphics2D g2d = (Graphics2D) g.create();
        g2d.scale(0.5, 0.5); // Уменьшаем в 2 раза для иконки

        if (isAlive) {
            drawTowerBody(g2d, (int)(x + xOffset), (int)y);
        } else {
            // Рисуем разрушенную башню
            g2d.setColor(Color.GRAY);
            g2d.fillRect((int)(x + xOffset) - 25, (int)y - 50, 50, 50);
        }
